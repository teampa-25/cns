...
from .icra2021 import ICRA2021


def compute_train_loss(net: ICRA2021, data, device):
    """
    Arguments:
    - net: torch.nn.Module
    ...
    """
    image_current, image_target, gt_vel, _ = [d.to(device) for d in data]
    pred_vel = net({"cur_img": image_current, "tar_img": image_target})
    result = net.criterion(pred_vel, gt_vel)
    loss = result["total_loss"]

    # add prefix
    result = {("train/" + k): v.item() for k, v in result.items()}

    # train_loader.feedback(gt_vel * 2, norm=False)
    train_loader.feedback(pred_vel * 2, norm=False)

    return result, loss, True


def compute_eval_score(net: ICRA2021, data, device):
    """
    Arguments:
    - net: torch.nn.Module
    - data: data from (for data in dataloader)
    ...
    pred_vel = net({"cur_img": image_current, "tar_img": image_target})
    result = net.criterion(pred_vel, gt_vel)
    no_weight_loss = result["no_weight_loss"]

    # add prefix
    result = {("valid/" + k): v.item() for k, v in result.items()}

    # valid_loader.feedback(gt_vel * 2, norm=False)
    valid_loader.feedback(pred_vel * 2, norm=False)

    return result, no_weight_loss.item(), False
...
if __name__ == "__main__":
    ...
    optimizers = {
        "0": optim.Adam(model.trainable_parameters(), lr=1e-4),
        "1": optim.Adam(model.criterion.parameters(), lr=5e-4)
    }
