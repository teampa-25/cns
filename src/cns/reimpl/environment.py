...
class ImageEnv(object):
    ...
    def __init__(self, camera_config, resample=True, auto_reinit=True, verbose=False, aug=False):
        ...
        self.initial_pose_r = [0.6, 0.9]
        self.initial_pose_phi = [60, 90]
    ...
    def get_warp_image(self, image_load: torch.Tensor, wcT):
        ...
        top_down_wcT[:3, 3] = np.array([0, 0, max(self.target_pose_r) * 1.2])
        # top_down_wcT[:3, 3] = np.array([0, 0, max(self.target_pose_r) * 1.5])
        ...
        M = K.geometry.get_perspective_transform(
    points_src.contiguous(), points_dst.contiguous()
).to(self.device)
    ...
    def aug_image(self, image: torch.Tensor):
        B, C, H, W = image.size()
        ...
        augs = [
            ...
            A.RandomPlasmaContrast(roughness=(0.1, 0.7), same_on_batch=False, keepdim=False, p=1.0)
        ]
    ...
    def observation(self):
        image_current = self.get_warp_image(self.image_load, self.current_wcT)
    ...
    def abnormal_pose(self):
        ...
        if pos[-1] < min(self.initial_pose_r) * 0.1:
            if self.verbose:
        ...
        x = (0-z0)/c*a + x0
        y = (0-z0)/c*b + y0
        if np.sqrt(x**2 + y**2) > max(self.target_pose_r):
