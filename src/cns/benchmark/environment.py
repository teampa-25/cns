...
class BenchmarkEnvAffine(reimpl_env.ImageEnvGUI):
    ...
    def __init__(self, scale=1.0, aug=False, one_image=None,custom_image_dir=None):
        self.client = p.connect(p.GUI)
        ...
        if custom_image_dir is not None:
            ...
            self.image_files = sorted(
                glob.glob(os.path.join(custom_image_dir, "*.png")) +
                glob.glob(os.path.join(custom_image_dir, "*.jpg")) +
                glob.glob(os.path.join(custom_image_dir, "*.jpeg")))
            num_images = len(self.image_files)
            print(f"[DEBUG] {num_images} Immagini trovate: {len(self.image_files)}")
        else:
            num_images = 150  # fallback
