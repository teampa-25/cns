...
class GraphVSController(object):
    def __init__(self, ckpt_path: str, device="cuda:0"):
        ...
        if "net" in ckpt:
            if isinstance(ckpt["net"], dict):
                self.net = GraphVS(2, 2, 128, regress_norm=True).to(device)
                self.net.load_state_dict(ckpt["net"])
            elif isinstance(ckpt["net"], GraphVS):
                self.net = ckpt["net"].to(device)
            else:
                raise TypeError(f"Unsupported type for 'net' in checkpoint: {type(ckpt['net'])}")
        else:
            self.net = GraphVS(2, 2, 128, regress_norm=True).to(device)
